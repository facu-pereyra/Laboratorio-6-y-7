...
/**  NO SE INCLUYE EL CERO A LA IZQUIERDA
  ...
  **/
uint8_t HoraInicio = 15;
uint8_t MinInicio  = 0;

uint8_t HoraFinal = 15;
uint8_t MinFinal  = 1;
...
char binName[24] = FILE_BASE_NAME "_00000000_000000.bin";
...
ISR(ADC_vect) {
  ...
  //}
    ...
    digitalWrite(LED_Work,   LOW); // Ver esta parte
  }
}
//------------------------------------------------------------------------------
...
void logData() {
  ...
  if (!binFile.contiguousRange(&bgnBlock, &endBlock))	LedError();
  ...
  if (bn != FILE_BLOCK_COUNT) {
    if (!binFile.truncate(512L * bn))	LedError();
  }

  if (!binFile.rename(sd.vwd(), binName))	LedError();
