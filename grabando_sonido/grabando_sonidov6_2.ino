...
uint8_t HoraInicio = 20;
uint8_t MinInicio  = 33;
uint8_t SecInicio  = 0;

uint8_t HoraFinal = 20;
uint8_t MinFinal  = 34;
uint8_t SecFinal  = 0;
...
int SaltoFichero_min=0.2; //Está en minutos
int SaltoFichero=6000; //Está en segundos
volatile boolean Siguiente = false; //para ir al próximo fichero
volatile boolean ControladorFinal=false; //Para que pare de grabar cuando se toque el boton
volatile boolean Grabando=false;
...
//------------------------------------------------------------------------------
void getFileName(){
    //char fn[30]; //Ni idea para que funciona
    DateTime now = rtc.now();
    //char foldername[] = "0000000";
    //sprintf(foldername, "%02d%02d%02d", now.year(), now.month(), now.day());
    //sprintf(foldername, "%02d%02d", now.month(),now.day());
    //SD.mkdir(foldername);
...
void loop() {
  ...
  if (((millis() - t) >SaltoFichero )&&(ControladorFinal==1)){        
      digitalWrite(LED_Work, LOW);
      Serial.println("Estoy en el salto de fichero");
      Siguiente=true;
      Grabando=false;
