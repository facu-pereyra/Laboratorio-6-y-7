...
Puse la condicion de cambio de fichero pero no corta normalmente, entonces no corta con la condicion de hora. El bucle es un count con while
*/
...
void loop() {
  ...
  if (!digitalRead(BotonStart)||(Siguiente==1)||(now.hour()==HoraInicio && now.minute()==MinInicio && now.second()==SecInicio)) { 
    ...
    while(count<21){
     ...
     delay(1000);
     Serial.println(count);
     if(!digitalRead(BotonBreak)||(now.hour()==HoraFinal && now.minute()==MinFinal && now.second()==SecFinal)){      
      LedFinal();     
...
//  else if((now.minute()-MinGrab)>1){ // La parte que esta adentro del parentesis es la q hay q modificar
//    digitalWrite(LED_Work, LOW);
//    Siguiente=true;
//    count=0;
//    audio.stopRecording(filename);
